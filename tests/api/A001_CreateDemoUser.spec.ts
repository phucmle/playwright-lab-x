...
const generateRandomUserName = () => `test_user_${Math.floor(Math.random() * 1000000)}`;
...
test.describe('Create new demo account via API', () => {
  test('Create new account successfully', async ({ request }) => {
    ...
    const { response, jsonResponse } = await createDemoAccount(request, newUserName);
  ...
  test('Cannot create with duplicated username', async ({ request }) => {
    const { response, jsonResponse } = await createDemoAccount(request, 'test_user243324');
